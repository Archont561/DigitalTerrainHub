...
declare module "alpinejs" {
    interface Alpine {
        ajaxPlugin: typeof AlpineAjaxPlugin;
    }
    interface Magics<T> {
        $ajax: InstanceType<typeof AlpineAjaxPlugin.AjaxRequestBuilder>;
    }
}
...
const AlpineAjaxPlugin = AlpinePluginBuilder.create<AjaxPluginSettings, AjaxAlpineElement>("ajaxPlugin")
.extend(plugin => {
    ...
    return {
        ...
        createAjaxHandler(el: AjaxAlpineElement, method: HTTPMethod) {
            const target = el._x_ajax_target || el;
            const swapStrategy = el._x_ajax_swap_strategy || pluginSettings.swapStrategy;
            const headers = Object.assign({}, this.DEFAULT_AJAX_HEADERS, el._x_ajax_headers || {}, pluginSettings.headers);
            ...
            return async (ajaxURL: string) => {
                ...
                try {
                    ...
                    if (!response.ok) {
                        if (!this.dispatch(el, this.AjaxEvent.Failure, { detail: { response } })) return;
                    } else {
                        if (!this.dispatch(el, this.AjaxEvent.Success, { detail: { response } })) return;
        ...
        async swap(el: HTMLElement, strategy: SwapStrategy, htmlString: string) {
            if (pluginSettings.transitions && document.startViewTransition) {
        ...
        performSwap(el: HTMLElement, strategy: SwapStrategy, htmlString: string) {
            switch (strategy) {
                case "before":
        ...
        dispatch(from: HTMLElement | Window, eventName: string, eventDict: Object = {}) {
            return from.dispatchEvent(new CustomEvent(eventName, eventDict));
        },
        getDelay(delayModifier?: string): number {
            if (!delayModifier) return pluginSettings.defaultListenerDelay;

            const match = /^(\d+)(ms|s)?$/.exec(delayModifier);
            if (!match) return pluginSettings.defaultListenerDelay;
    ...
    callback: (el,
        ...
        if (!value) {
            ...
            source.onerror = error => {
                if (!plugin.dispatch(el, 'ajax-sse:error', { detail: { error } })) return;
            ...
            const handler = (() => {
                ...
                if (!isExpressionEmpty) return (event: MessageEvent) => evaluate(
                    expression, { scope: { $sse: event, $swap: plugin.swap } });

                if (hasTarget) return (event: MessageEvent) => plugin.swap(
                    el._x_ajax_target as HTMLElement,
                    el._x_ajax_swap_strategy as SwapStrategy,
                    event.data
                );

                return (event: MessageEvent) => plugin.swap(el, "replace", event.data);
            })();
    ...
    callback: (el,
        ...
        el._x_ajax_swap_strategy = (modifiers.at(0) || plugin.getSettings().swapStrategy) as SwapStrategy;
    }
})).addDirective(plugin => ({
    name: "ajax-headers",
    callback: (el, { expression }, { evaluate }) => {
        el._x_ajax_headers = JSON.parse(evaluate(expression));
    }
})).addDirective(plugin => ({
    name: "ajax-values",
    callback: (el, { expression, value }, { evaluate }) => {
    ...
    callback: (el,
        ...
        const ajaxHandler = plugin.createAjaxHandler(el, method.toUpperCase() as HTTPMethod);
        ...
        if (value) {
            ...
            if (value === "load") {
                ...
                const delay = plugin.getDelay(modifiers.find(modifier => /^\d+(ms|s)?$/.test(modifier)));
    ...
    name: "ajax",
    callback: (el, { Alpine }) => new plugin.AjaxRequestBuilder(el, Alpine, plugin.createAjaxHandler)
}));

export default AlpineAjaxPlugin;